...
# create plot
pdf("fig2_peaks_with_1_repeat_barplot.pdf", width = 8, height = 4.5)
...
peak_repeats %>% filter(Sample=="TERRA" & Peak_Group!="All") %>% filter(grepl("(No_R|Min_1R)",Repeat_Group,perl = T)) %>% 
  ...
  facet_wrap(~factor(Peak_Group, levels = unique(Peak_Group)), scales="free_x") + 
  ...
  scale_fill_manual(values=cbPalette, labels = c('Repeats = 0','Repeat >= 1'))
